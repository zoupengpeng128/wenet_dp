...
from wenet.transformer.ctc import CTC, CTCPrompt
...
from wenet.utils.checkpoint import load_checkpoint, load_trained_modules, save_checkpoint
from wenet.transformer.asr_model import init_asr_model as init_asr_model_source
from wenet.utils.file_utils import read_symbol_table,read_symbol_table_int2char
...
class ASRModel(torch.nn.Module):
    ...
    def __init__(
        ...
        if symbol_table != '':
            self.symbol_table = read_symbol_table_int2char(symbol_table)
    def forward(
        self,
        speech: torch.Tensor,
    ...
    def load_lfmmi_resource(self):
        ...
        with open('{}/words.txt'.format(self.lfmmi_dir), 'r') as fin:
            for line in fin:
                arr = line.strip().split()
                assert len(arr) == 2
                self.word_table1[int(arr[1])] = arr[0]
                self.word_table2[arr[0]] = int(arr[1])
    ...
    def _calc_lfmmi_loss(self, encoder_out, encoder_mask, text):
        ...
        for i in text:
            ...
            for j in i:
                if j.item() != -1:
                    temp_char = self.symbol_table[j.item()]
                    if temp_char not in self.word_table2:
                        temp_char = '<UNK>'
                    list_t.append(temp_char)
            temp.append(' '.join(list_t))
        text = temp
    ...
    def hlg_onebest(
        ...
        for i in hyps:
            word_info_list=[]
            for j in i:
                word_info_list.append(self.word_table[j])
    ...
    def hlg_rescore(
        ...
        """
        r_decoder_scores = decoder_scores
...
def update_from_other_model(model, configs):
    other_configs = None
    with open(configs['from_other_model']['yaml_path'], 'r') as fin:
            other_configs = yaml.load(fin, Loader=yaml.FullLoader)

    trained_model = init_asr_model_source(other_configs)
    load_checkpoint(trained_model, configs['from_other_model']['pt_path'])
    model = load_trained_modules(model, trained_model, ['ctc.ctc_lo.weight', 'ctc.ctc_lo.bias'],'',None)
    #model = load_trained_modules(model, trained_model, [], '', None)
    return model

def init_asr_model_from_trained(configs):
    if configs['cmvn_file'] is not None:
        mean, istd = load_cmvn(configs['cmvn_file'], configs['is_json_cmvn'])
        global_cmvn = GlobalCMVN(
            torch.from_numpy(mean).float(),
    ...
    if configs['from_other_model'] is not None:
        other_configs = None
        with open(configs['from_other_model']['yaml_path'], 'r') as fin:
            other_configs = yaml.load(fin, Loader=yaml.FullLoader)

        trained_model = init_asr_model(other_configs)
        load_checkpoint(trained_model, configs['from_other_model']['pt_path'])
    model = load_trained_modules(model, trained_model, [], 'encoder.embed.embed_domain.weight', source_embed)
    ...
    for name, value in model.named_parameters():
        print(name)
        if name.find("domain_prompt_module") == -1 and name not in ["ctc.ctc_lo.weight", "ctc.ctc_lo.bias"]:
            value.requires_grad = False
            num_source_params += value.numel()
        else:
            print("*********use********")
            print(name)
            update_params +=value.numel()
            if name in ["ctc.ctc_lo.weight", "ctc.ctc_lo.bias",
                        "ctc.ctc_linear1.weight", "ctc.ctc_linear1.bias", "ctc.ctc_linear2.weight"]:
                num_source_params += value.numel()
                ctc_params += value.numel()
        num_all_params += value.numel()
    print("all_params: %d"%(num_all_params))
    print("num_source_params: %d"%(num_source_params))
    print("update_params: %d"%(update_params))
    print("ctc_params: %d"%(ctc_params))
